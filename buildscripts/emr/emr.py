...
bucket = simples3.S3Bucket(settings.emr_bucket,
                           settings.emr_id,
                           settings.emr_key)
...
def _get_status():
    ...
    return "{}-{}".format(
        utils.execsys("git describe")[0].strip(), gh(["diff", "status"]))
...
def get_build_info():
    return (os.environ.get('MONGO_BUILDER_NAME'),
            os.environ.get('MONGO_BUILD_NUMBER'))


def make_tarball():
    m = _get_most_recent_tgz("mongodb-")
    c = "test-code-emr.tgz"
    tar = "tar zcf {} src jstests buildscripts".format(c)
...
def push():
    ...
    root = "emr/{}/{}".format(datetime.date.today().strftime("%Y-%m-%d"),
                              os.uname()[0].lower())
    ...
    def make_long_name(local, hash):
        ...
        return "{}/{}-{}.{}".format(root, pcs[0], h, pcs[2])
...
def run_tests(things, tests):
    ...
    print("tests expanded from {} to {}".format(oldNum, len(tests)))
    ...
    def http(path):
        return "http://{}.s3.amazonaws.com/{}".format(settings.emr_bucket,
                                                      path)

    run_s3_path = "emr/{}/{}/{}/".format(
                            os.getenv("USER"),
                            os.getenv("HOST"),
                            datetime.datetime.today().strftime("%Y%m%d-%H%M"))

    run_s3_root = "s3n://{}/{}/".format(settings.emr_bucket, run_s3_path)

    out = run_s3_root + "out"
    logs = run_s3_root + "logs"

    jar = "s3n://{}/{}".format(settings.emr_bucket, things[2])
    step_args = [http(things[0]), http(things[1]), out, ",".join(tests)]

    step = boto.emr.step.JarStep("emr main", jar=jar, step_args=step_args)
    print("jar:{}\nargs:{}".format(jar, step_args))

    setup = boto.emr.BootstrapAction(
        "setup", "s3n://{}/{}".format(settings.emr_bucket, things[3]), [])

    jobid = emr.run_jobflow(
        name="Mongo EMR for {} from {}".format(
                        os.getenv("USER"),
                        os.getenv("HOST")),
        ec2_keyname="emr1",
    ...
    print("{} jobid: {}".format(datetime.datetime.today(), jobid))

    while (True):
        flow = emr.describe_jobflow(jobid)
        print("{} status: {}".format(datetime.datetime.today(), flow.state))
...
def do_output(dir):
    ...
    if do_credentials():
        ...
        if builder and buildnum:
            ...
            if doc:
                print("\nhttp://buildlogs.mongodb.org/build/{}".format(
                                                                doc["_id"]))
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        build_jar()
        cmd = ["java", "-cp",
            os.environ.get("CLASSPATH", ".") + ":emr.jar", "emr"]
